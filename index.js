...
say('ROBÔ ALPHA\nV1', {
  ...
  say(`ROBÔ ALPHA By @${author.name || author}`, {
    font: 'console',
    align: 'center',
    gradient: ['red', 'green']
  })
Drips.hr();
console.log(color(''), color('OBRIGADO POR ESCOLHER O ROBÔ ALPHA', 'green'))
console.log(color(''), color('SCRIPT BY DRIPS', 'red'))
console.log(color( ''), color('https://wa.me/244936222026','cyan'))
Drips.hr();
Drips.banner(`ALPHA PROPRIETÁRIO DO BOT`)
Drips.ok('BEM-VINDO AO ROBÔ ALPHA')
Drips.done('EU ESCREVI ESTE SCRIPT POR MIM MESMO')
Drips.info('QUER CONTRIBUIR SINTA-SE A VONTADE PARA ENTRAR EM CONTATO, SER AMIGÁVEL É A MINHA LINGUAGEM')
Drips.error('')
Drips.time('')
Drips.hr();
 ...
 ZimBotInc.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (db.chats[m.chat].isWelcome) {//bem-vindo por gotas
                if (anu.action == 'add') {
                var buffer = await getBuffer(ppuser)
                var wangu = await getBuffer(picak+'BEM VINDO CHOMIE')
                let fgclink = {key: {fromMe: false,"participant":"0@s.whatsapp.net", "remoteJid": "6289523258649-1604595598@g.us"}, "message": {orderMessage: {itemCount: 9999999,status: 200, thumbnail: buffer, surface: 200, message: `${metadata.subject}`, orderTitle: 'memek', sellerJid: '0@s.whatsapp.net'}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true}
                he = `Bem-vindo ao ${metadata.subject} @${num.split("@")[0]}\n\n${metadata.desc}`
...
mentions: [num],
fileName: `BEM-VINDO AO ${metadata.subject}`,
...
ZimBotInc.sendMessage(anu.id, buttonMessage)
                    ...
                    var buffer = await getBuffer(ppuser)
                    var mhatadzenyu = await getBuffer(picak+'ADEUS CHOMIE')
                    let fgclink = {key: {fromMe: false,"participant":"0@s.whatsapp.net", "remoteJid": "6289523258649-1604595598@g.us"}, "message": {orderMessage: {itemCount: 9999999,status: 200, thumbnail: buffer, surface: 200, message: `${metadata.subject}`, orderTitle: 'memek', sellerJid: '0@s.whatsapp.net'}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true}
                    he = `Ele/Ela se foi mano ${metadata.subject} @${num.split("@")[0]}\n\n${metadata.desc}`
                    
let buttons = [
{buttonId: `halo`, buttonText: {displayText: 'ADEUS'}, type: 1}
]
let buttonMessage = {
document: fs.readFileSync('./lib/tes.xlsx'),
mimetype: feler,
jpegThumbnail:buffer,
mentions: [num],
fileName: `Ele/Ela se foi mano ${metadata.subject}`,
fileLength: 99999999999999,
caption: he,
footer: `©ALPHA 2022`,
buttons: buttons,
...
//settings
    ...
    ZimBotInc.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output?.statusCode
            if (reason === DisconnectReason.badSession) { console.log(`Arquivo de sessão inválido, exclua a sessão e verifique novamente`); process.exit(); }
            else if (reason === DisconnectReason.connectionClosed) { console.log("Conexão encerrada, reconectando...."); startZimBotInc(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Conexão perdida do servidor, reconectando..."); startZimBotInc(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Conexão substituída, outra nova sessão aberta, feche a sessão atual primeiro"); process.exit(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`Dispositivo desconectado, exclua a sessão e digitalize novamente.`); process.exit(); }
            else if (reason === DisconnectReason.restartRequired) { console.log("Reiniciar Necessário, Reiniciando..."); startZimBotInc(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Tempo esgotado da conexão, conectando..."); startZimBotInc(); }
            else { console.log(`Motivo Desconectado Desconhecido: ${reason}|${connection}`) }
        }
        console.log('ROBÔ ALLPHA...', update)
    })
